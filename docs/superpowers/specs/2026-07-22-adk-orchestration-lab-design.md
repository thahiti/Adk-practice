...
| 2 | 한 요청에 여러 서브에이전트 호출 필요 | 다중홉 eval 케이스 + ReAct 오케스트레이션 (§6.1 결정 기록 참고) |
...
| F17 | ~~`PlanReActPlanner`는 프롬프트 기반이라 모델 비종속이다~~ **정정(2026-07-23):** docstring이 보장하는 것은 "built-in thinking 불필요"뿐. 주입 메커니즘은 모델 비종속이나 프로토콜의 효과는 모델 종속 — OpenAI에서 위임 0회 실측. §6.1 결정 기록 참고 | `planners/plan_re_act_planner.py:35`, 실측 |
...
오케스트레이션은 **ReAct(관찰-행동 반복) 단일 방식**이다. 배선을 바꾸지 않고 `config.py`가 주입하는 모델 값 하나만 교체한다. 에이전트 구조, 위임 계약, 평가 하네스는 어느 백엔드에서도 동일하다.

### 6.1 결정 기록: PlanReActPlanner 제거 (2026-07-23)

초기 설계는 `ORCHESTRATION_MODE` 토글로 `PlanReActPlanner`를 붙여 ReAct와 Plan-Execute를 A/B 비교하려 했으나, 실측 후 제거했다.

**실측 결과:** OpenAI 백엔드 × plan_execute 조합에서 다중홉 케이스 전체가 위임 0회로 실패했다. 저장된 결과를 뜯어보면 모델이 `/*PLANNING*/` 블록에 계획을 산문으로 서술하고 function call 없이 턴을 끝냈고, ADK의 `is_final_response()`(FC 없으면 턴 종료)에 의해 invocation이 즉시 종료됐다.

**근본 원인:** `PlanReActPlanner`는 실행 엔진이 아니라 프롬프트 주입기 + 응답 라벨러다. 계획을 파싱·실행하는 코드는 없고, instruction이 "tool **code snippets**... Python snippets... e.g., `vertex_search.search()`"라고 코드 스타일 액션을 지시한다. 이는 텍스트 태그와 네이티브 function call을 한 응답에 혼합하는 Gemini 관행을 전제한 프로토콜이며, gpt-4o는 지시를 문자 그대로 따라 텍스트만 낸다. ADK 자체 유닛테스트도 instruction 주입 여부만 검증할 뿐 모델이 이 프로토콜에서 실제로 툴을 부르는지는 테스트하지 않으며, 저장소 내 예제는 Gemini 전용이다.

**F17 정정:** "프롬프트 기반이라 모델 비종속"은 *주입 메커니즘*에 대해서만 참이다. docstring이 보장하는 것은 "built-in thinking 기능이 필요 없다"뿐이며, *프로토콜의 효과*는 모델 종속이다. 백엔드 축과 오케스트레이션 축이 독립적으로 조합된다는 4셀 매트릭스 가정은 이 지점에서 성립하지 않았다.

OpenAI에서 계획 효과를 측정하려면 OpenAI-네이티브 지시문을 쓰는 커스텀 `BasePlanner`가 필요하다. 이는 본 랩의 비목표로 둔다.
...
백엔드 2종 각각에 대해 `delegation_route_score`와 `tool_trajectory_avg_score` 두 점수가 나온다. 이 표를 채우는 것이 랩의 산출물이다.

| | ReAct |
|---|---|
| **`vertex`** (gemini-2.5-flash) | route / trajectory |
| **`openai`** (gpt-4o) | route / trajectory |

**검증할 가설:** 라우팅 정확도는 두 모델이 비슷하지만, 다중홉 파라미터 정확도에서는 차이가 벌어진다. 가설이 틀릴 수 있으며, 틀렸다는 사실 자체가 유효한 측정 결과다.
...
for backend in vertex openai; do
  MODEL_BACKEND=$backend uv run pytest -m requires_model
done
```
...
| 모델이 다중홉에서 중간 결과를 잘못 전달 | `tool_trajectory_avg_score` 하락 | 실패가 아니라 측정 결과로 기록. 백엔드 간 비교가 실험 목적 |
...
- [ ] 백엔드 2종(vertex / openai)의 두 점수가 README에 비교표로 기록된다
