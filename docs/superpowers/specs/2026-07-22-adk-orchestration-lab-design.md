...
- **LLM:** Vertex AI `gemini-2.5-flash` — 접근 **가능**
- **일반 인터넷:** 접근 **불가**
- 따라서 "오프라인 테스트"란 *툴이 외부 API에 의존하지 않는다*는 뜻이며, LLM을 mock으로 대체한다는 뜻이 아니다. 모든 동작과 평가는 실제 모델로 수행한다.
...
배선을 바꾸지 않고 루트의 `planner` 필드만 교체한다.

| 모드 | 설정 | 의미 |
|---|---|---|
| `react` | `planner=None` | 계획 없이 관찰-행동 반복 |
| `plan_execute` | `planner=PlanReActPlanner()` | 계획 선언 후 실행 (F9) |

환경 변수 `ORCHESTRATION_MODE`로 선택하며 기본값은 `react`. `config.py`가 읽어 `agent.py`에 주입한다.

동일한 eval셋을 두 모드로 실행해 위임 정확도를 비교하는 것이 이 랩의 핵심 실험이다. 요구사항 2가 "ReAct 또는 Plan-Execute"로 열려 있으므로, 어느 쪽이 나은지를 실측으로 답한다.

**검증할 가설:** 다중홉에서 중간 결과를 다음 위임의 인자로 넘겨야 할 때 `plan_execute`가 `react`보다 파라미터 정확도가 높을 것이다.
...
| `requires_vertex` | 실제 모델 호출. 평가 계층 | 필요 |

```bash
uv run pytest -m offline            # 네트워크 없이 즉시 실행
uv run pytest -m requires_vertex    # ADC 필요
```
...
cp .env.example .env    # GOOGLE_CLOUD_PROJECT 채우기
...
# 위임 평가 (ADC 필요)
uv run pytest -m requires_vertex

# 두 모드 비교
ORCHESTRATION_MODE=react        uv run pytest -m requires_vertex
ORCHESTRATION_MODE=plan_execute uv run pytest -m requires_vertex
```
...
`google-adk[eval]==1.36.1` — 평가 모듈이 `pandas` 등을 요구하므로 `eval` extra가 필수다 (F12).
...
| 리스크 | 영향 | 대응 |
|---|---|---|
| **ADC 미설정** | L2/L3 전면 실행 불가 | 착수 전 `gcloud auth application-default login` 필요. L1과 오프라인 계층은 무관하게 선행 가능 |
...
- [ ] `react`와 `plan_execute` 두 모드의 두 점수가 README에 비교표로 기록된다
